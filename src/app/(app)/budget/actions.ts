...
export async function saveBudgetLine(
  ...
  if (!can.manageSubscriptions(user)) return { error: 'ບໍ່ມີສິດຕັ້ງງົບປະມານ' }
...
export async function deleteBudgetLine(
  ...
  const user = await requireUser()
  if (!can.manageSubscriptions(user)) return { error: 'ບໍ່ມີສິດລຶບ' }
...
export async function addBudgetSpend(
  ...
  const user = await requireUser()
  if (!can.manageSubscriptions(user)) return { error: 'ບໍ່ມີສິດບັນທຶກລາຍຈ່າຍ' }
