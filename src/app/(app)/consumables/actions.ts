...
export async function createConsumable(
  ...
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດເພີ່ມລາຍການ' }
...
export async function updateConsumable(
  ...
  const user = await requireUser()
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດແກ້ລາຍການ' }
...
export async function moveConsumable(
  ...
  const user = await requireUser()
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດບັນທຶກການເຄື່ອນໄຫວ' }
...
export async function deleteConsumableMove(
  ...
  const user = await requireUser()
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດລຶບ' }
