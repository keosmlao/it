...
export async function saveSegment(
  ...
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດແກ້ຂໍ້ມູນເຄືອຂ່າຍ' }
...
export async function saveIpAssignment(
  ...
  const user = await requireUser()
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດແກ້ທະບຽນ IP' }
...
export async function saveSwitchPort(
  ...
  const user = await requireUser()
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດແກ້ຜັງພອດ' }
