...
export async function createSubscription(
  ...
  if (!can.menu(user, '/subscriptions/new', 'create')) return { error: 'ບໍ່ມີສິດລົງທະບຽນສັນຍາເຊົ່າ' }
...
export async function updateSubscription(
  ...
  const user = await requireUser()
  if (!can.menu(user, '/subscriptions', 'edit')) return { error: 'ບໍ່ມີສິດແກ້ສັນຍາເຊົ່າ' }
...
export async function cancelSubscription(
  ...
  const user = await requireUser()
  if (!can.menu(user, '/subscriptions', 'delete')) return { error: 'ບໍ່ມີສິດຍົກເລີກສັນຍາ' }
...
export async function reopenSubscription(
  ...
  const user = await requireUser()
  if (!can.menu(user, '/subscriptions', 'edit')) return { error: 'ບໍ່ມີສິດເປີດສັນຍາຄືນ' }
...
export async function recordPeriod(
  ...
  const user = await requireUser()
  if (!can.menu(user, '/subscriptions', 'edit')) return { error: 'ບໍ່ມີສິດບັນທຶກການຈ່າຍ' }
...
export async function setPeriodStatus(
  ...
  const user = await requireUser()
  if (!can.menu(user, '/subscriptions', 'edit')) return { error: 'ບໍ່ມີສິດແກ້ງວດການຈ່າຍ' }
...
export async function deletePeriod(
  ...
  const user = await requireUser()
  if (!can.menu(user, '/subscriptions', 'delete')) return { error: 'ບໍ່ມີສິດລຶບງວດ' }
...
export async function renewSubscription(
  ...
  const user = await requireUser()
  if (!can.menu(user, '/subscriptions', 'edit')) return { error: 'ບໍ່ມີສິດຕໍ່ອາຍຸ' }
