...
export async function createMaintenancePlan(
  ...
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດຕັ້ງແຜນບຳລຸງຮັກສາ' }
...
export async function updateMaintenancePlan(
  ...
  const user = await requireUser()
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດແກ້ແຜນ' }
...
export async function logMaintenance(
  ...
  const user = await requireUser()
  if (!can.manageAssets(user)) return { error: 'ບໍ່ມີສິດບັນທຶກ' }
